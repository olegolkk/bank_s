...
#ifdef VERSION
#ifdef DEBUG
    wch << " Version (Debug) SHA   : " + GIT_VERSION + ENDLINE
#else
    wch, << Version(Release) SHA : " + GIT_VERSION + ENDLINE
#endif
        wch << " Release Date and Time : " + GIT_TIME_RELEASE + ENDLINE
        wch << ENV_LINES + ENDLINE
        wch << " Build Date and Time   : " + GIT_TIME_BUILD + ENDLINE
        wch << ENV_LINES + ENDLINE
#endif
}
...
*/
void ENV_StopByError(INT32 iError, VECTOR<STRING>* error_message = NULL, FSTREAM* FileName = NULL)
{
    std::cout << " " << ENDLINE;
    std::cout << "ERROR CODE: " << iError << ENDLINE;
    if (FileName)
    {
        *FileName << " " << ENDLINE;
        *FileName << "ERROR CODE:" << iError << ENDLINE;
    }
    if (error_message)
        for (auto emi = error_message->begin(); emi != error_message->end(); ++emi)
        {
            if (emi->length())
            {
                std::cout << *emi << ENDLINE;
                if (FileName) *FileName << *emi << ENDLINE;
            }
        }
    exit(iError);
}
...
void ENV_PrintWarning(VECTOR<STRING>* warning_message = NULL, FSTREAM* FileName = NULL)
{
    if (warning_message)
    {
        for (auto wmi = warning_message->begin(); wmi != warning_message->end(); ++wmi)
            if (wmi->length())
            {
                std::cout << *wmi << ENDLINE;
                if (FileName) *FileName << *wmi << ENDLINE;
            }
    }
}
...
{
    return v(v.cbegin() + m, v.cbegin() + (n + 1));
}
...
static size_t max_line_length(VECTOR<STRING> const& lines)
{
